...
target_time = np.datetime64("2016-04-02T18:00")
lon_min, lon_max = 87.0, 117.0
lat_min, lat_max = 9.9, 35.1
out_dir = "E:/WRF/WRFs/era0402m2"
...
def main():
    ...
    for p in plevs:
        ...
        ax.add_feature(
            ...
            facecolor=(0.86, 0.93, 0.98, 0.6),   # RGBA：浅蓝色
            edgecolor='none',
            zorder=0
        )
...
# ===========================
        #画位势高度等值线
        cs = ax.contour(
            Lon, Lat, Z_smooth, levels=12, colors="darkgrey", linewidths=0.8, transform=ccrs.PlateCarree()
        )
        ax.clabel(cs, fmt="%d", fontsize=14)

        # ---- 右下角风矢图例加白色底板 ----
        #key_bg = mpatches.FancyBboxPatch(
        #    (0.68, 0.005),   # 左下角位置（axes坐标）
        #    0.35, 0.09,      # 白底板的宽和高
        ...
        q = ax.quiver(
            ...
            color='black',
            transform=ccrs.PlateCarree(),
            scale=400,          # 用于调整风向箭头大小（长度）
            width=0.003,       # 新增：让箭杆变细
            headwidth=4,      # 新增：箭头头部也略微变细
            headlength=6,
            headaxislength=4.0,
            zorder=4
        )
		...
		#修改输出文件名
        outfn = os.path.join(out_dir, f"era5_20160402_18z_{int(p)}hPa_05.png")  #输出文件名
